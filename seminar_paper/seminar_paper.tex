...
\begin{algorithmic}[1]
	...
	\State $f(u,v) \gets 0$ $\forall e=(u,v) \in E$ \Comment{Start with a valid flow}
	\State  $G_r \gets G$ 
	\State $c_r(P) \gets min \{c_r(u,v): (u,v) \in P(s,t) \}$
	\While{$\exists P(s,t) \in G_r: \forall (u,v) \in P(s,t): c_r(u, v) > 0$}
		\State $f(u,v) \gets f(u,v) + c_r(P)$ \Comment{Increase the flow by as much flow as can be pushed along $P(s,t)$}
...
\label{sec:dinitz}
$N = (V,E,s,t,c)$ is a \textit{layered network}~\cite{yossi81} if each vertex $v \in V$ has a layer number $l(v)$ s.t.
\begin{itemize}
	\item $l(s) := 0$ and $0 \leq l(v) \leq l(t)$ $\forall v \in V$
	\item $e = (u,v) \in E  \Rightarrow l(v) - l(u) = 1$
\end{itemize}
Figure \ref{fig:dinitz} shows a layered network and its underlying directed acyclic network.
...
\end{tikzpicture}
\end{center}
\caption{A layered network (in blue) with layer numbers and its underlying network}
\label{fig:dinitz}
\end{figure}
...
\begin{algorithmic}[1]
	...
	\While{$l(t) \neq \infty$} %\Comment{\textcolor{OliveGreen}{$O(n)$}}	
		...
		\State $f_l \gets$ MAX-FLOW($G_l$)  %\Comment{tbd}
		\State $f \gets f$ + $f_l$ %\Comment{\textcolor{OliveGreen}{$O(n)$}}
	\EndWhile
	\EndFunction
\end{algorithmic}
\end{algorithm}
...
\begin{itemize}
	...
	\item Each flow quantum $Q$ has a processor $P(Q)$ assigned. In other words, each leaf in each of the $2n \cdot d_{\mathrm{in}}(v)$ T-IN(v) PS-trees has a processor $P(Q)$ assigned.
\end{itemize}
...
\begin{algorithmic}[1]
	...
	\State $\forall 1 \leq j \leq k: S(j) \gets \Sigma_{i=1}^{j} size(a_i)$
	\State $\forall 1 \leq j \leq l: R(j) \gets \Sigma_{i=1}^{j} c_r(v,w_i)$
	...
	\If{$a_i$ assigned to more than 1 $(v,w_j)}$
		\ForAll{$(v,w_j)$ assigned to $a_i$}
			\If{assignment saturates $(v,w_j)$}
				...
				\State $size(a_k) \gets$ quanta assigned to $(v,w_j)$
				\State $trace(a_k) \gets trace(a_i)$ 
				\State $position(a_k) \gets position(a_i)$
			\EndIf
		\EndFor
	\EndIf	
	\ForAll{$(v,w_j)$}
		\ForAll{$a_i \leftrightarrow (v,w_j)$}
			\State $f(v,w_j) \gets f(v,w_j) + size(a_i)$
			\State $location(a_i) \gets w_j$
			\State $trace(a_i) \gets trace(a_i) + w_j$
		\EndFor 
	\EndFor
	...
	\ForAll{$w \in V$ and $w$ blocked}
		\ForAll{$a_i$ at $w$}
			...
			\State $f(v_{a_i}, w) \gets f(v_{a_i}, w) - size(a_i)$
			\State $location(a_i) \gets v_{a_i}$
			\State $trace(a_i) \gets trace(a_i) - (v_{a_i}, w)$
		\EndFor
	\EndFor	
	\EndFunction
\end{algorithmic}
...
\hline
1 & Ford-Fulkerson~\cite{ahuja93} & $O((n+m) \cdot f_{max})$ & Sequential algorithm \\
2 & Edmonds-Karp~\cite{ahuja93} & $O(n\cdot m^2)$ & Sequential algorithm  \\
3 & Dinitz~\cite{dinitz70}& $O(n^2 \cdot m)$ & Sequential algorithm \\
  & & & on layered networks\\
4 & Preflow-Push~\cite{ahuja93} & $O(n^2\cdot m)$, $O(nm \log(n^2/m))$ & Various improvements\\
