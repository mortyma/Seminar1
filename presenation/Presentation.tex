...
\framesubtitle{The INITIALIZE function}
	\begin{algorithmic}[1]
	\Function{INITIALIZE}{$N=(V,A,s,t,c)$}
...
\pause
\bigskip
Repeat PUSH until all atoms at $s$ or $t$
\end{frame}
...
\begin{itemize}
	\item atoms can move forward on $(v,w) \in E$, increasing the $f(v,w)$
	\item or backward on $(w,v)$, decreasing the flow. $w$ becomes blocked. 
...
\hline
\rowcolor{c1} Ford-Fulkerson & $O((n+m) * f_{max})$ & Sequential algorithm \\
\rowcolor{c2} Edmonds-Karp & $O(n*m^2)$ & Sequential algorithm  \\
\rowcolor{c1} Dinitz& $O(n^2*m)$ & Sequential algorithm on layered networks\\
\rowcolor{c2} Preflow-Push & $O(n^2*m)$, $O(n*m*log(n^2/m))$ & Various improvements\\
\rowcolor{c1} Shiloach-Vishkin & $O(n^2*log m)$, $p=O(n)$ & on CREW PRAM\\
\rowcolor{c2} Goldberg-Tarjan & $O(n*log n)$, $p=O(m)$, $O(nm)$ space & on EREW PRAM \\
\rowcolor{c1} Vishkin & $O(n*log n)$, $p=O(n)$, $O(n^2)$ space & Combines previous two\\
\rowcolor{c2} Johnson & $O(log^{3} n)$, $p=O(n^{4})$ or $O(log^{2} n)$, $p=O(n^{6})$ &
for planar networks\\
