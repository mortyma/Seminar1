...
\end{itemize}
\pause
Apparently large number of processors can be reduced to $n$\footfullcite{yossi81}.
\end{frame}
...
\pause
\textcolor{red}{Result}: solves Max-Flow in $O({n}^{2} * log nt)$ time using $n$ processors on a CREW PRAM.
\end{frame}
...
\pause
\bigskip
Repeat PUSH until all atoms at arrive $s$ or $t$
\end{frame}
